...
# --- objects with no ground-truth mask (DECISION_unpaired_annotations.md) ---
...
def test_gt_without_mask_is_excluded_not_scored_zero():
    """The core guarantee: an object whose mask GT does not exist must not be
    charged as a failure. Contrast with add_missed_gt, which must be."""
    excluded = SegmentationEvaluator(num_classes=1)
    excluded.add_pair(0, _full(), _full())        # perfect
    excluded.add_gt_without_mask(0)               # no mask GT

    missed = SegmentationEvaluator(num_classes=1)
    missed.add_pair(0, _full(), _full())          # perfect
    missed.add_missed_gt(0)                       # real detection failure

    e, m = excluded.summarize()["overall"], missed.summarize()["overall"]
    assert e["gt_anchored_iou"] == pytest.approx(1.0)   # untouched
    assert m["gt_anchored_iou"] == pytest.approx(0.5)   # penalised
    assert e["tp_only_iou"] == m["tp_only_iou"] == pytest.approx(1.0)


def test_excluded_count_is_reported_not_silent():
    ev = SegmentationEvaluator(num_classes=1)
    ev.add_pair(0, _full(), _full())
    for _ in range(3):
        ev.add_gt_without_mask(0)
    s = ev.summarize()
    assert s["overall"]["n_excluded_no_mask_gt"] == 3
    assert s["excluded_no_mask_gt"]["count"] == 3
    assert s["excluded_no_mask_gt"]["fraction_of_gt_objects"] == pytest.approx(3 / 4)
    assert s["per_class"]["plane"]["n_excluded_no_mask_gt"] == 3


def test_class_appears_even_if_only_excluded_objects():
    ev = SegmentationEvaluator(num_classes=2)
    ev.add_gt_without_mask(1)
    s = ev.summarize()
    assert s["per_class"]["ship"]["n_excluded_no_mask_gt"] == 1
    assert s["per_class"]["ship"]["n_matched"] == 0


def test_no_excluded_objects_leaves_summary_unchanged_in_shape():
    ev = SegmentationEvaluator(num_classes=1)
    ev.add_pair(0, _full(), _full())
    s = ev.summarize()
    assert s["excluded_no_mask_gt"]["count"] == 0
    assert s["overall"]["gt_anchored_iou"] == pytest.approx(1.0)

...
**Last updated: 2026-08-04 (by ATHENA, NTUA dev PC — roadmap v2.4 fast track)**
...
| **Dataset scope: AerialFuseCV = exact pairs only** | One dataset, one job: detector → DOTA, segmentation → iSAID, unified detect→segment → AerialFuseCV. Keeping unpaired objects was considered and rejected: it does not preserve data (those objects remain in DOTA/iSAID in full — exclusion defines a scope rather than destroying anything), and it would complicate a published artefact for a filtered variant users can build themselves. Pairs-only also eliminates at source the hazard of scoring a prediction against an absent ground truth. Known cost: ~2.1% of DOTA boxes are dropped, so a detector finding one is charged an FP — answered by scope (measure detectors on DOTA) and constant across compared methods. **Every discarded object is logged with its reason** (`discarded.jsonl` + statistics); accounting is exact and asserted per build. Full argument and reviewer challenges: **`documentation/DECISION_unpaired_annotations.md`** | 2026-07-30 |
...
| **iSAID baseline literature does not invalidate ArgusVision; it narrows the claim** | iSAID already has supervised instance-segmentation baselines and SAM/open-vocabulary remote-sensing work, so ArgusVision must not claim novelty as "first iSAID segmentation" or "first SAM in remote sensing." The defensible need is the paired object-level detection-to-segmentation contract: using DOTA-style detector geometry and detector outputs as prompts for foundation segmentation, measuring detector error and mask quality together, and comparing that training-free pathway against supervised baselines. Full positioning note: `ArgusVision_Strategic_Planning/Context/iSAID_baselines_positioning_NOTES.md` | 2026-08-04 |
| **Roadmap → v2.3: compete on contract + failure science, not SAM-wrapper novelty** | Competitive review (2026-08-04): RSPrompter/SAM-RSIS/OBSeg/SOPSeg-ReSOS/InstructSAM/AerOSeg dense on methods; ArgusVision durable assets are pairing contract, protocol divergence, cascade evaluator. P1 argument → protocol-aware descriptor; P2 → cascade reliability (experiment block unchanged); parked OBB reframed (cannot claim first OBB prompts — OBSeg/SOPSeg); candidates C1 protocol-divergence letter / C2 auto-mask quality / C3 optional systems split remain unscheduled. P0–P4 countable sequence stands. Full notes: `ArgusVision_Strategic_Planning/Context/competitive_landscape_2026-08_NOTES.md` + `PUBLICATION_ROADMAP.md` v2.3 | 2026-08-04 |
| **Claim landmines locked** | Never claim: first SAM in RS; first iSAID segmentation; first OBB prompts for SAM; YOLO+SAM novelty; iSAID lacks baselines. Preferred language: paired detect→segment contract; protocol reconciliation; cascade error decomposition; training-free vs supervised tradeoff when masks are scarce | 2026-08-04 |
| **Roadmap → v2.4: FAST TRACK** | User directive 2026-08-04: publish fast; most science done. Critical path F1–F10 only. Dates: P0 ≤ 08-22, P1 ≤ 09-05, P2 experiments ≤ 09-15, P2 submit ≤ 2027-01-31, P3 → IGARSS Jan. Freeze P4/C1–C3/parked until P1 submitted + P2 experiments done. OPEN 6 largely mitigated by `dataset/build_aerialfusecv.py`; remaining P0 = deposit-scoped script + package + licensing MVP. Need statement locked in roadmap | 2026-08-04 |
| **Multi-model: locks yes, brand matrix no; F1 single-threaded** | Coordination via `ACTIVE_TASKS.md` is mandatory. Control files are the collision surface. Orchestrator = repo-connected session that reads protocol files — context continuity beats vendor model tables. F1→F4 serial; genuine parallel now = licensing admin + reviewer-only. Code parallelism at F6–F8. Claim rule wired into RISE v2.1.1 + bootstrap stubs | 2026-08-04 |

---

## 🔄 IN PROGRESS

- **FAST TRACK active (roadmap v2.4).** Immediate work: F1 deposit rebuild script → F2 Zenodo → F3 licensing MVP → F4/F5 P1 DiB submit ≤ 2026-09-05. Parallel after P0: F6 OPEN 5 Mask R-CNN path → F7–F8 P2 experiments ≤ 2026-09-15.
- **P1 writing** — drafts exist; rewrite to protocol spine + `results/` numbers once DOI is live.
- **Restructure residuals** — deferred under fast track (do not block submissions).
- Frozen until P1 submitted + P2 experiments done: P4, C1–C3 as separate papers, parked papers.
...
6. **P0 remaining work (OPEN 6 largely mitigated).** `dataset/build_aerialfusecv.py` is a one-pass internal builder — Stages 1+3 are no longer missing as code. Still required for Zenodo: **deposit-scoped** rebuild script + verification against the internal build, packaging, checksums, README, LICENSE, v1.0 label (OPEN 7). Licensing MVP = reference DOTA/iSAID images, do not redistribute; chase NTUA office in parallel.
...
| 2026-08-04 | NTUA dev PC, iSAID baseline positioning | Logged the strategic answer to the iSAID-baseline threat question. Existing supervised segmentation, SAM-adaptation, and open-vocabulary iSAID work narrows but does not kill ArgusVision. The durable positioning is dataset/evaluator/methodological need: paired object-level detector geometry plus masks, rigorous detector-to-segmenter evaluation, and training-free-vs-supervised tradeoff analysis. |
| 2026-08-04 | NTUA dev PC, competitive review → roadmap v2.3 | Deep competitive search (RSPrompter, SAM-RSIS, OBSeg, SOPSeg/ReSOS, InstructSAM, AerOSeg/OV lines, YOLO+SAM). Roadmap → v2.3: P1 protocol spine, P2 cascade framing, OBB reframe, candidates C1–C3, claim landmines. Context note `competitive_landscape_2026-08_NOTES.md`. P0–P4 sequence unchanged. |
| 2026-08-04 | NTUA dev PC, fast track → roadmap v2.4 | User directive: publish fast. Roadmap → v2.4 with F1–F10 critical path, week-by-week sprints, compressed dates, freeze on P4/candidates. OPEN 6 status updated (builder exists; deposit packaging remains). Need statement locked. |
| 2026-08-04 | NTUA dev PC, ACTIVE_TASKS + claim discipline | Created `ACTIVE_TASKS.md`; wired claim step into RISE + identical bootstrap stubs; corrected MULTI_MODEL_ORCHESTRATION parallelism (F1 single-threaded; reviewer-only parallel). F1 claimed by Cursor-Orchestrator. |

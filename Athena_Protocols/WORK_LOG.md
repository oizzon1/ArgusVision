...
### 2026-07-31 — Mode(s): OPERATOR / REVIEWER / ADVISOR — **SESSION ONGOING**

| Field | Entry |
|---|---|
| Work hours | ~05:40 → ongoing EEST (first commit 07:46; earlier work uncommitted) |
...
| Next actions | Per the agreed step list in `TODO_AERIALFUSECV_REBUILD.md`: measure the discrepancy split (`extra` vs `missing`, foreign-instance pixels, multi-piece instances) → fix our extraction → define and build reconciled masks → re-run the experiment → EDA → deliverable script → clean work area → re-source P1. Licensing answers still outstanding (NTUA, source authors, venue enquiry). |

"""
Convert DOTA v1 original format to YOLO VBB (Vertical/Axis-Aligned Bounding Box) format.
Converts oriented boxes to axis-aligned boxes (center-x, center-y, width, height).
"""
...
def convert_obb_to_aabb(x1, y1, x2, y2, x3, y3, x4, y4):
    """
...
def convert_label_file(original_label_path, output_label_path, image_path):
    """Convert a single label file from DOTA to YOLO VBB format."""
    ...
    for line in lines:
        ...
        try:
            ...
            cx, cy, w, h = convert_obb_to_aabb(x1, y1, x2, y2, x3, y3, x4, y4)
            ...
            output_line = f"{class_id} {cx_norm:.6f} {cy_norm:.6f} {w_norm:.6f} {h_norm:.6f}\n"
...
def convert_dataset():
    """Convert entire DOTA v1 dataset to YOLO VBB format."""
    source_root = Path("dataset/DOTA_v1")
    target_root = Path("dataset/DOTA_v1_YOLO_vertical_bboxes_dataset")
    
    print("=" * 80)
    print("DOTA v1 to YOLO VBB Conversion")
    print("=" * 80)
